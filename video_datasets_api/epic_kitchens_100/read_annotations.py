...
def epic_narration_id_to_unique_id(train_pkl, val_pkl, test_pkl = None):
    ...
    narration_ids_sorted = sorted(train_labels.narration_id, key = sort_narration_id)

    with open(val_pkl, 'rb') as f:
        val_labels = pickle.load(f)

    val_narration_ids = sorted(val_labels.narration_id, key = sort_narration_id)
    ...
    if test_pkl is not None:
        ...
        test_narration_ids = sorted(test_labels.narration_id, key = sort_narration_id)
...
def get_verb_uid2label_dict(train_pkl, val_pkl, narration_id_to_video_id):
    ...
    for index in range(num_videos):
        narration_id = train_labels.narration_id.iloc[index]
    ...
    for index in range(num_videos):
        narration_id = val_labels.narration_id.iloc[index]
        uid = narration_id_to_video_id[narration_id]
