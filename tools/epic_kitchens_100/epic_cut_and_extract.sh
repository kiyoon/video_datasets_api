...
if [ $# -lt 4 ]
then
	echo "usage: $0 [input_dir_55 (should be .../videos containing train and test folder)] [input_dir_extension] [output_dir] [annotations_root_dir] [quality=5 (2(best)-31(worst))]"
...
do
	...
	do
		...
		output_path="$output_dir/$split/$id"
		mkdir -p "$output_path"
		echo "Using $video_path -> Output $output_path"
		
		ffmpeg -ss $start_time -i "$video_path" -t $duration_sec -vf scale=-2:324 -sws_flags bicubic -an -start_number 0 -qscale:v "$quality" "$output_path/%05d.jpg" < /dev/null 2> /dev/null
