...
if [[ $# -lt 4 ]]
then
	echo "Usage: $0 [EPIC-55 dir] [EPIC-100 extensions dir] [output_dir] [md5.csv]"
	echo "Find tar file from either of the two directory structures, and extract them in one location."
...
do
	...
	mkdir -p "$extract_dir"
	if tar xf "$epic55_dir/$tarfile" -C "$extract_dir"
	then
		true
	else
